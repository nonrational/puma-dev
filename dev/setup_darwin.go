...
`
	...
	err = os.WriteFile(
		plist,
		fmt.Appendf(nil, userTemplate, binPath, dir, config.Domains, config.Timeout, config.NoServePublicPaths, config.ListenPort, config.TlsPort, logPath, logPath),
		0644,
	)
...
func Uninstall(launchAgentDirPath string, domains []string) func() {
	...
	files, err := os.ReadDir("/etc/resolver/")
	if err != nil {
		fmt.Printf("! Unable to read /etc/resolver/: %s\n", err)
		return func() {}
	}
	...
	for _, file := range files {
		path := filepath.Join("/etc/resolver", file.Name())
		...
		if strings.Contains(string(content), "Generated by puma-dev") {
	...
	if success {
		fmt.Printf("* Removed all Puma-dev generated files from /etc/resolver\n")
	}
